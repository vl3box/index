<template>
    <div class="m-calendar">
        <div class="m-calendar-content">
            <div class="m-calendar-item" :class="{ current: isToday(item) }" v-for="(item, index) in data" :key="index">
    ...
    methods: {
        ...
        loadRangeCalendar() {
            getRangeCalendar(this.params).then(res => {
                ...
                if (data) {
                    data?.forEach(item => {
                        const index = this.data.findIndex(v => v.year === item.year && v.month === item.month && v.date === item.date);
